...
label = ["PVL-first,salary$\in$[5k,15k]", "PVL-first,salary$\in$[10k,20k]", "[15],salary$\in$[5k,15k]","[15],salary$\in$[10k,20k]"]
...
input_path = r'competitior_1d_q1.csv'
input_file = open(input_path, "r")
Lines = input_file.readlines()
# Strips the newline character
for line in Lines:
...
lgnd = plt.legend([handles[idx] for idx in order],[labels[idx] for idx in order],
                  loc='upper center', bbox_to_anchor=(0.42, 1.6), fontsize=44, ncol=2, labelspacing=0.06,
                  handletextpad=0.1, markerscale=0.1, columnspacing=0.08, handlelength=1.1, handleheight=0.6, frameon=False)

plt.savefig("comparision_1d_q1q2.png", bbox_inches='tight')
plt.show()
