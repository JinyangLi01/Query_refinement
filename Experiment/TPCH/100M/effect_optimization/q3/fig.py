...
#
# x_list = ['\\boldmath$Q^{T,3}_1$\\boldmath$C^{T,3}_1$', '\\boldmath$Q^{T,3}_1$\\boldmath$C^{T,3}_2$',
#           '\\boldmath$Q^{T,3}_2$\\boldmath$C^{T,3}_1$', '\\boldmath$Q^{T,3}_2$\\boldmath$C^{T,3}_2$']
x_list = ['\\boldmath$Q^{T}_3$\\boldmath$C^{T,3}_1$', '\\boldmath$Q^{T}_3$\\boldmath$C^{T,3}_2$']


plt.xticks(np.arange(0, 2) + bar_width/2, x_list, rotation=0, fontsize=65)
plt.yticks(fontsize=80, weight='bold')

plt.xlabel('Query and Constraint', fontsize=80, weight='bold')

plt.yscale('log')
lgnd = plt.legend(loc='upper right', bbox_to_anchor=(1.04, 0.93), fontsize=65, ncol=1, labelspacing=0.4,
                  handletextpad=0.1, markerscale=0.5, columnspacing=0.5)

plt.tight_layout()
plt.savefig("TPCH100M_optimization_effect_q3.png", bbox_inches='tight')
plt.show()
