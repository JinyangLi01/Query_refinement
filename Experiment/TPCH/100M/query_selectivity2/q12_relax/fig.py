...
def run(query, size, constraint):
    ...
    plt.xlabel(r'l\underline{ }receiptdate starting from\\$~$10/01/1994, 4m interval', fontsize=77,
               weight='bold', labelpad=-5).set_position((0.44, -0.1))

    plt.tight_layout()
    # plt.legend(loc='upper right', bbox_to_anchor=(1.03, 1.05))
    plt.legend(loc='upper left', bbox_to_anchor=(-0.03, 1.08), fontsize=65,
               ncol=1, labelspacing=0.1, handletextpad=0.2, markerscale=0.1,
               columnspacing=0.2, borderpad=0.2, frameon=True, handlelength=1.7)
