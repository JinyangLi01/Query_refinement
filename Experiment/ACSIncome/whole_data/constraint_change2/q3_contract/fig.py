...
def run(query, constraint):
    ...
    plt.xticks(np.arange(0, 6), x_list, rotation=0, fontsize=85)
    plt.yticks(fontsize=85, weight='bold')

    plt.xlabel(r'\{race=White\} $<=$ (\%)',
               fontsize=80, weight='bold', labelpad=-10).set_position((0.46, -0.1))
    plt.tight_layout()
    plt.legend(loc='upper left', bbox_to_anchor=(0.2, 0.8), fontsize=65,
