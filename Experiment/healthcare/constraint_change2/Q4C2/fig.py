...
def run(query, constraint):
    ...
    plt.xticks(np.arange(0, 6, 1), x_list, rotation=0, fontsize=75)
    plt.yticks(fontsize=80, weight='bold')

    plt.xlabel(r'\{smoker=True\} $<=$ (\%)',
               fontsize=80, weight='bold', labelpad=-10).set_position((0.40, -0.1))
    plt.tight_layout()
    plt.legend(loc='upper left', bbox_to_anchor=(0, 0.6), fontsize=60,
